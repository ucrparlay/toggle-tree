...
run_bfs() {
    ...
    (
        cd "${BFS_DIR}"
        PARLAY_NUM_THREADS="${threads}" numactl -i all ./bfs -i "${BIN_DIR}${graph}.bin" -n "${NUM_ROUNDS}"
...
run_kcore() {
    ...
    (
        cd "${KCORE_DIR}"
        PARLAY_NUM_THREADS="${threads}" numactl -i all ./kcore -s -i "${BIN_DIR}${graph}.bin" -n "${NUM_ROUNDS}"
    ) | tee /dev/stderr | extract_time
}
