...
class KCore {
 ...
 public:
  ...
  // Process neighbors of vertex u, updating degrees and frontier/buckets
  ...
  // Process neighbors without bucketing (for single-bucket phase)
      ...
      while (k < bucketing_pt) {
        ...
        while (peeling_frontier.advance_to_next()) {
          ...
          if (counting_flag) {
            ...
            t_check_n_count.stop();
            // Move any newly discovered k-vertices from bucket to frontier
            buckets[k & bucket_mask].template for_each<false>([&](size_t v) {
              if (coreness[v] == k) {
                buckets[k & bucket_mask].remove(v);
                peeling_frontier.insert_next(v);
              }
            });
          }
    ...
    if (remaining_vertices.size() > 0) {
      for (NodeId base_k = 0;; base_k += stride) {
        ...
        for (NodeId k = base_k; k < base_k + stride; k++) {
          ...
          if (base_k != k) {
            for (int i = num_intermediate_buckets - 1; i >= 0; i--) {
              ...
              if ((k & mask) == 0) {
                ...
                t_pack.start();
                // Redistribute intermediate bucket into finer buckets
                buckets[num_single_buckets + i].for_each<true>(
                    [&](size_t u) {
          ...
          t_dump.stop();
          t_push.start();
          // Move matching vertices from bucket to frontier
          buckets[k & bucket_mask].template for_each<false>([&](size_t v) {
            if (coreness[v] == k) {
              buckets[k & bucket_mask].remove(v);
              peeling_frontier.insert_next(v);
