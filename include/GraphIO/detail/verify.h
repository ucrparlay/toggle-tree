...
void process_result(const char* dumppath, const char* filepath, double t, T& result, bool print = false, std::string gbbs_path = "") {
    ...
    uint32_t maxi = 0;
    for (uint32_t i=0; i<res_32.size(); i++) if (res_32[i] != UINT32_MAX && res_32[i] != INT32_MAX && res_32[i] > maxi) maxi = res_32[i];
    if (print) std::cout << "Maximum Res : " << maxi << "\n";

    std::string get_hash = GetHash(res_32);
    if (print) std::cout << "Verify  Hash: " << get_hash << '\n';

    if(graph_name.find("FiveStarRedFlag") == std::string::npos){
        if (gbbs_path == ""){
