...
struct ParallelBitmap {
    ...
    inline void insert(size_t v) noexcept {
        for (int8_t i=5; i>=0; i--) { if (__atomic_fetch_or(&bitmap[i][v>>off(i)], (1ULL<<((v>>off(i+1)) & 63)), __ATOMIC_RELAXED)) return; }
    }
    ...
    inline void for_each(int8_t layer, uint64_t base, F&& f) {
        uint64_t mask = bitmap[layer][idx(layer, base)];
        if constexpr (Remove) bitmap[layer][idx(layer, base)] = 0;
        if (layer == 5) {
            parlay::parallel_for(0, __builtin_popcountll(mask), [&](uint64_t i) {
                f(base + (__builtin_ctzll(__builtin_ia32_pdep_di(1ULL << i, mask)) << off(layer + 1)));
            },Gran);
        }
        else {
            parlay::parallel_for(0, __builtin_popcountll(mask), [&](uint64_t i) {
                for_each<Remove, Gran>(layer + 1, base + (__builtin_ctzll(__builtin_ia32_pdep_di(1ULL << i, mask)) << off(layer + 1)), f);
            },1);
        }
    }
    template <bool Remove, size_t Gran, class F>
    ...
    inline T reduce(int8_t layer, uint64_t base, F&& f, Combine&& combine) {
        ...
        T best = Identity;
        if (layer == 4) {
            for(uint64_t i=0;i<__builtin_popcountll(mask);i++){
                uint64_t childbase = base+(__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<i,mask))<<off(5));
                uint64_t m = bitmap[5][idx(5,childbase)];
                T b = Identity;
                for(uint64_t j=0;j<__builtin_popcountll(m);j++) b = combine(b,f(childbase+__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<j,m))));
                best = combine(best,b);
            }
        }
        else {
            ...
            parlay::parallel_for(0, __builtin_popcountll(mask), [&](uint64_t i){
                bests[i] = reduce<T, Identity>(layer+1, base+(__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<i,mask))<<off(layer+1)), f, combine);
            },1);
            for(uint64_t i=0;i<__builtin_popcountll(mask);i++) best=combine(best,bests[i]);
        }
        return best;
    }
    template<class T, auto Identity, class F, class Combine>
    ...
    inline uint64_t reduce_vertex(int8_t layer, uint64_t base) {
        ...
        if (layer == 3) {
            for(uint64_t i=0;i<__builtin_popcountll(mask);i++){
                uint64_t childbase = base+(__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<i,mask))<<off(4));
                uint64_t childmask = bitmap[4][idx(4,childbase)];
                for(uint64_t j=0;j<__builtin_popcountll(childmask);j++){
                    sum += __builtin_popcountll(bitmap[5][idx(5,childbase+(__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<j,childmask))<<off(5)))]);
                }
            }
        }
        else {
            ...
            parlay::parallel_for(0, __builtin_popcountll(mask), [&](uint64_t i){
                sums[i] = reduce_vertex(layer+1, base+(__builtin_ctzll(__builtin_ia32_pdep_di(1ULL<<i,mask))<<off(layer+1)));
            },1);
            for(uint64_t i=0;i<__builtin_popcountll(mask);i++) sum += sums[i];
        }
        return sum;
    }
